...
These thresholds are not a single industry constant, they vary by data center and vendor standards. It helps to think of the synchronous, zero data loss tier as one application-layer redundancy model with its own envelope. That tier covers tightly coupled microservices, strict transactional banking, and synchronous database clustering like Oracle RAC or MongoDB replica sets that need split-second consistency, and the published envelopes for it generally sit under 5 to 10 ms round trip and a maximum separation in the rough range of 500 to 1000 km. The 3ms and 300km figures above are a conservative point inside that band. Where a given platform draws the line depends on how chatty its replication protocol is and how much write latency the application can absorb, so the right number is the one your specific system and its standard allow, not a figure you can copy from someone else's design.
